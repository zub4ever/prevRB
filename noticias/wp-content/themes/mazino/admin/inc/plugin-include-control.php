...
if (class_exists('WP_Customize_Control') && !class_exists('Mazino_Plugin_Install_Control')) {
	/**
	 *
	 * @see WP_Customize_Section
	 */
	class Mazino_Plugin_Install_Control extends WP_Customize_Control {
		/**
		 ...
		 * @var string
		 */
		public $type = 'mazino_plugin_install_control';
		public $name = '';
		public $slug = '';
		...
		public  function enqueue() {
			...
			wp_enqueue_script('mazino-companion-install', get_stylesheet_directory_uri() . '/assets/js/plugin-install.js', array('jquery'));
		...
		protected function render_content() {
			...
			$hide_install = get_option('mazino_hide_customizer_notice_'.$this->slug,  false);
			if($hide_install){
				return;
			}

			global $mazino_about_page;
			if(!is_object($mazino_about_page)){
				return;
			}
			
			$path = $mazino_about_page->get_plugin_basename_from_slug($this->slug);
			if($mazino_about_page->is_plugin_active($path)){
				return;
			}
			?>
